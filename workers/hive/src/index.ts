...
function corsHeaders(origin: string): Record<string, string> {
  return {
    ...
    "Access-Control-Allow-Headers":
      "Content-Type, Authorization, X-Request-ID, X-Internal-Secret",
...
async function handleHealth(env: Env): Promise<Response> {
  const { data } = await d1Query(
    env.DB,
    "SELECT COUNT(*) as count FROM hive_nodes WHERE status = ?",
    ["active"],
  );
...
async function handleRoute(request: Request, env: Env): Promise<Response> {
  const rl = await checkRateLimit(
    env.KV_RATE_LIMIT,
    `route:${request.headers.get("CF-Connecting-IP")}`,
    100,
    60,
  );
  if (!rl.allowed)
  ...
  const payloadHash = await crypto.subtle
    .digest("SHA-256", new TextEncoder().encode(JSON.stringify(body.payload)))
    .then((buf) =>
  ...
  const { data: nodes } = await d1Query<{
    node_id: string;
    load_score: number;
  }>(
    env.DB,
...
async function handleGetMessage(
  ...
  const { data } = await d1Query(
    env.DB,
    "SELECT * FROM hive_message_log WHERE message_id = ?",
    [messageId],
  );
  if (!data?.length)
...
async function handleListChannels(env: Env): Promise<Response> {
  const { data } = await d1Query(
    env.DB,
    "SELECT * FROM hive_channels ORDER BY created_at DESC LIMIT 100",
  );
...
async function handleGetChannel(
  ...
  const { data } = await d1Query(
    env.DB,
    "SELECT * FROM hive_channels WHERE channel_id = ?",
    [channelId],
  );
  if (!data?.length)
    return errorResponse("Channel not found", 404, "NOT_FOUND");
  return jsonResponse(data[0]);
}
...
async function handleDeleteChannel(
  ...
  const { data } = await d1Query(
    env.DB,
    "SELECT channel_id FROM hive_channels WHERE channel_id = ?",
    [channelId],
  );
  if (!data?.length)
    return errorResponse("Channel not found", 404, "NOT_FOUND");
  await d1Run(
    env.DB,
    "UPDATE hive_channels SET status = ?, updated_at = ? WHERE channel_id = ?",
    ["closed", new Date().toISOString(), channelId],
  );
...
async function handleListNodes(env: Env): Promise<Response> {
  const { data } = await d1Query(
    env.DB,
    "SELECT * FROM hive_nodes ORDER BY load_score ASC",
  );
...
async function handleGetNode(nodeId: string, env: Env): Promise<Response> {
  const { data } = await d1Query(
    env.DB,
    "SELECT * FROM hive_nodes WHERE node_id = ?",
    [nodeId],
  );
...
async function handleNodeHealth(nodeId: string, env: Env): Promise<Response> {
  const { data } = await d1Query<{
    ...
    env.DB,
    "SELECT node_id, status, load_score, last_heartbeat FROM hive_nodes WHERE node_id = ?",
    [nodeId],
  );
  if (!data?.length) return errorResponse("Node not found", 404, "NOT_FOUND");
  const node = data[0];
...
async function handleTopology(env: Env): Promise<Response> {
  const [nodesResult, channelsResult, msgResult] = await Promise.all([
    d1Query<{ role: string; status: string }>(
      env.DB,
      "SELECT role, status FROM hive_nodes",
    ),
    d1Query<{ status: string; classification: string }>(
      env.DB,
      "SELECT status, classification FROM hive_channels",
    ),
    d1Query<{ status: string; latency_ms: number }>(
      env.DB,
      "SELECT status, latency_ms FROM hive_message_log WHERE created_at > ?",
  ...
  return jsonResponse({
    topology: {
      nodes: {
        total: nodes.length,
        by_role: nodes.reduce((acc: Record<string, number>, n) => {
      ...
      messages_1h: {
        ...
        avg_latency_ms: messages.length
          ? Math.round(
              messages.reduce((s, m) => s + (m.latency_ms ?? 0), 0) /
                messages.length,
            )
          : 0,
      },
...
async function handleInternalHeartbeat(
  request: Request,
  env: Env,
): Promise<Response> {
  ...
  const now = new Date().toISOString();
  const { data: existing } = await d1Query(
    env.DB,
  ...
  if (existing?.length) {
    await d1Run(
      env.DB,
      "UPDATE hive_nodes SET status = ?, load_score = ?, last_heartbeat = ?, updated_at = ? WHERE node_id = ?",
...
export default {
  async fetch(
    ...
    try {
      ...
      if (method === "GET" && path.match(/^\/route\/[\w-]+$/))
        return handleGetMessage(path.split("/")[2], env);
      ...
      if (method === "GET" && path.match(/^\/channels\/[\w-]+$/))
        return handleGetChannel(path.split("/")[2], env);
      if (method === "DELETE" && path.match(/^\/channels\/[\w-]+$/))
        return handleDeleteChannel(path.split("/")[2], env);

      // Nodes
      if (method === "GET" && path === "/nodes") return handleListNodes(env);
      if (method === "GET" && path.match(/^\/nodes\/[\w-]+\/health$/))
        return handleNodeHealth(path.split("/")[2], env);
      if (method === "GET" && path.match(/^\/nodes\/[\w-]+$/))
        return handleGetNode(path.split("/")[2], env);

      // Topology & discovery
      if (method === "GET" && path === "/topology") return handleTopology(env);
      if (method === "GET" && path.match(/^\/discover\/[\w-]+$/))
        return handleDiscover(path.split("/")[2], env);

      // Internal
      if (method === "POST" && path === "/internal/heartbeat")
        return handleInternalHeartbeat(request, env);
